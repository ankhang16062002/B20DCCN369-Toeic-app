...
export default function Choices({ A0, A1, A2, A3, correct, correctAnswers }) {
  ...
  const [selectedChoice, setSelectedChoice] = [
    useSelector((state) => state.quiz.selectedChoice),
    (payload) => dispatch(SetSelectedChoice(payload)),
  ];
  ...
  const setChoiceHandler = (choice) => {
    ...
    if (isTraversing && selectedChoices[shownQuestion] == choice) {
      setIsCorrect(
        selectedChoices[shownQuestion] == correctAnswers[shownQuestion] ? 1 : 0
      );
  ...
  const answerStyles = (
    ...
    defaultStyle,
    selectStyle,
    traverseStyleCorrect,
    traverseStyleIncorrect,
    border = null
  ) => {
    return [
      defaultStyle,
      selectedChoice == choice && selectStyle,
      selectedChoices[shownQuestion] == choice &&
        isTraversing &&
        (choice == correctAnswers[shownQuestion]
          ? traverseStyleCorrect
          : traverseStyleIncorrect),
      isTraversing && (choice == correctAnswers[shownQuestion] ? border : null),
    ];
  };
  ...
  return (
    <ScrollView style={[styles.answersContainer]}>
      <Pressable
        style={answerStyles(
          1,
          [styles.answer, { borderColor: colors.border }],
          [styles.selectedAnswer, { backgroundColor: colors.light }],
          styles.selectedAnswerCorrect,
          styles.selectedAnswerIncorrect,
          { borderColor: "#32c953", borderWidth: 3 }
        )}
        onPressIn={() => setChoiceHandler(1)}
      >
        <View>
          <Text
            style={answerStyles(
              1,
              [styles.letter, { color: colors.light }],
              [
                [styles.selectedLetter, { color: colors.dark }],
                { color: colors.dark },
              ],
              [styles.selectedLetterCorrection, { color: "white" }],
              [styles.selectedLetterCorrection, { color: "white" }]
            )}
          >
            A
        ...
        <Text
          style={answerStyles(
            1,
            [styles.answerText, { color: colors.light }],
            [styles.selectedAnswerText, { color: colors.dark }],
            [styles.selectedAnswerTextCorrection, { color: "white" }],
            [styles.selectedAnswerTextCorrection, { color: "white" }]
          )}
        >
          {A0}
