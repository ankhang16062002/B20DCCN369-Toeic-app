...
const EvalPopUp = ({ explanation, referTo, totalCount, correctAnswers }) => {
  ...
  const [currentQuestion, setCurrentQuestion] = [
    useSelector((state) => state.quiz.currentQuestion),
    (payload) => dispatch(SetCurrentQuestion(payload)),
  ];
  ...
  const [selectedChoices, setSelectedChoices] = [
    useSelector((state) => state.quiz.selectedChoices),
    (payload) => dispatch(SetSelectedChoices(payload)),
  ];

  const [modalVisible, setModalVisible] = [
    useSelector((state) => state.quiz.modalVisible),
    (payload) => dispatch(SetModalVisible(payload)),
  ];

  const [isCorrect, setIsCorrect] = [
    useSelector((state) => state.quiz.isCorrect),
    (payload) => dispatch(SetIsCorrect(payload)),
  ];
        ...
        >
          <View style={styles.centeredView}>
            <View
              style={[
                styles.modalView,
                { backgroundColor: colors.light, borderColor: "white" },
              ]}
            >
              {isCorrect == 1 ? (
                <>
                  ...
                  >
                    {!isTraversing ? "Đúng!" : "Đúng!"}
                  </Text>
                  {isTraversing ? (
                    <>
                      {explanation != "" ? (
                        <Text
                          style={[styles.modalText, { color: colors.dark }]}
                        >
                ...
                <>
                  ...
                  >
                    <Text style={{ fontFamily: "Poppins-Bold", fontSize: 17 }}>
                      {letters[correctAnswers[shownQuestion] - 1]}
                    </Text>{" "}
                    là câu trả lời đúng.
                  </Text>
                  {explanation ? (
              ...
              >
                <Text style={[styles.submitText, { color: colors.light }]}>
                  {!isTraversing && currentQuestion != totalCount - 1
                    ? "Tiếp theo"
                    : "Okay"}
                </Text>
              </Pressable>
